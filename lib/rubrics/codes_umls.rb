module FHIR
  class CodesUmls < FHIR::Rubrics
    ...
    # This method checks whether or not the coding is eligible and validate.
    ...
    # If the coding is eligible, then it is validate.
    def self.check_coding(coding,supposed_to_be_umls,declared_binding)
      ...
      if coding.nil?
        ...
        if supposed_to_be_umls || CODE_SYSTEMS[local_binding]
          result[:eligible_fields] += 1 
          in_umls = !FHIR::Terminology.get_description(CODE_SYSTEMS[local_binding],coding.code).nil?
    ...
    def self.eligible_binding(metadata)
      ...
      matching_binding = CODE_SYSTEMS[vs_binding['uri']] if !vs_binding.nil? && vs_binding['uri']

      valid_codes = metadata['valid_codes']
      matching_code_system = valid_codes.keys.find{|k| CODE_SYSTEMS[k]} if !valid_codes.nil?
      matching_code_system = CODE_SYSTEMS[matching_code_system] if matching_code_system
